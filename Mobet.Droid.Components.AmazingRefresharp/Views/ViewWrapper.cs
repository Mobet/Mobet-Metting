...
{
    public enum AmazingRefreshsharpRefreshState
    {
        PullToRefresh,
        ReleaseToRefresh,
        Refreshing,
    ...
    {
        private const string TAG = "AmazingRefreshsharpWrapper";

        private AmazingRefreshsharpRefreshState refresh_state = AmazingRefreshsharpRefreshState.PullToRefresh;
        ...
        public AmazingRefreshsharpRefreshState State {
			get {
				return refresh_state;
			}
		}

        public IAmazingRefreshsharpWrappedView ContentView {
            get;
            private set;
        }
        ...
        {
            TypedArray a = Context.ObtainStyledAttributes(attrs, Resource.Styleable.AmazingRefreshsharpWrapper, defStyle, 0);

            header_res_id = a.GetResourceId(Resource.Styleable.AmazingRefreshsharpWrapper_headerId, 0);
            content_view_res_id = a.GetResourceId(Resource.Styleable.AmazingRefreshsharpWrapper_contentViewId, 0);
            pulldown_progress_indicator_res_id = a.GetResourceId(Resource.Styleable.AmazingRefreshsharpWrapper_pullDownProgressIndicatorId, 0);

            pull_to_refresh_string_id = a.GetResourceId(Resource.Styleable.AmazingRefreshsharpWrapper_pullToRefreshText,
                                            Resource.String.ptrsharp_pull_to_refresh);
            release_to_refresh_string_id = a.GetResourceId(Resource.Styleable.AmazingRefreshsharpWrapper_releaseToRefreshText,
                                            Resource.String.ptrsharp_release_to_refresh);
            refreshing_string_id = a.GetResourceId(Resource.Styleable.AmazingRefreshsharpWrapper_refreshingText,
                                            Resource.String.ptrsharp_refreshing);

            header_background_res_id = a.GetResourceId(Resource.Styleable.AmazingRefreshsharpWrapper_ptrHeaderBackground, 0);
            header_text_color = a.GetColorStateList(Resource.Styleable.AmazingRefreshsharpWrapper_headerTextColor);

            pulldown_icon_drawable_res_id = a.GetResourceId(Resource.Styleable.AmazingRefreshsharpWrapper_headerIconDrawable, 0);

            pulldown_tension_factor = a.GetFloat(Resource.Styleable.AmazingRefreshsharpWrapper_pullDownTension, 0.5f);
            SnapbackDuration = a.GetInt(Resource.Styleable.AmazingRefreshsharpWrapper_snapbackDuration, 400);
            IsPullEnabled = a.GetBoolean(Resource.Styleable.AmazingRefreshsharpWrapper_pullEnabled, true);

            fastscroll_thumb_width = a.GetDimensionPixelSize(Resource.Styleable.AmazingRefreshsharpWrapper_fastScrollThumbWidth, -1);
        ...
        private void UpdateRefreshState(AmazingRefreshsharpRefreshState state)
        {
        ...
        public override bool OnInterceptTouchEvent(MotionEvent ev)
        {
            ...
            if (ev.ActionMasked == MotionEventActions.Down && ContentView.IsAtTop
                && refresh_state != AmazingRefreshsharpRefreshState.Refreshing) {
                ...
                ContentView.IgnoreTouchEvents = refresh_state == AmazingRefreshsharpRefreshState.Refreshing && current_scroll_y < 0;
                ...
                case MotionEventActions.Move:
                    ...
                    if (ContentView.IsAtTop) {
                        ...
                        if (Math.Abs(current_scroll_y) >= header_measured_height) {
                            if (refresh_state != AmazingRefreshsharpRefreshState.Refreshing) {
                                SetPullDownIconProgress(1);
                                UpdateRefreshState(AmazingRefreshsharpRefreshState.ReleaseToRefresh);
                            }
                        } else {
                            ...
                            if (refresh_state != AmazingRefreshsharpRefreshState.Refreshing) {
                                ...
                                UpdateRefreshState(AmazingRefreshsharpRefreshState.PullToRefresh);
                            }
                        }
        ...
        private bool HandleUpEvent(MotionEvent e)
        {
            ...
            if (refresh_state == AmazingRefreshsharpRefreshState.ReleaseToRefresh) {
                UpdateRefreshState(AmazingRefreshsharpRefreshState.Refreshing);
            }
        ...
        public void OnGlobalLayout()
        {
            ...
            if (header_measured_height == 0) {
                ...
                if (content_view_res_id > 0) {
                    ContentView = (IAmazingRefreshsharpWrappedView)FindViewById<View>(content_view_res_id);
                } else {
                    ContentView = (IAmazingRefreshsharpWrappedView)GetChildAt(1);
                }
